...
g3 = np.random.randn(N)
g4 = np.random.randn(N)
g5 = np.random.randn(N)
g6 = np.random.randn(N)
...
x = np.zeros((len(tau), N, len(x0)))
EL2_dat = np.zeros((len(tau), N))
...
for i in range(len(tau)):
    ...
    x[i] = Langevin2(x0, vel, gauss, tau[i], N)
    
    x1 = np.array([x[i,j,0] for j in range(N)])
    y1 = np.array([x[i,j,1] for j in range(N)])
    z1 = np.array([x[i,j,2] for j in range(N)])
    x2 = np.array([x[i,j,3] for j in range(N)])
    y2 = np.array([x[i,j,4] for j in range(N)])
    z2 = np.array([x[i,j,5] for j in range(N)])
    
    EL2_dat[i] = EL2(x1, y1, z1, x2, y2, z2)
    
EL2_arr = np.average(EL2_dat, axis=1)
err2 = np.std(EL2_dat, axis=1)/np.sqrt(N/48**2)
