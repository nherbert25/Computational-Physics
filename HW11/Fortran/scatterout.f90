...
real*8 pi,e0,v0,alpha,xint,x0,delx,tmax,delx2,dx,dk,dt,anorm,varg,s
!       parameter (hbar=1.d0, hbar2=1.d0, amass = 1.d0)  
       parameter (pi = 3.141592653589793d0, Ndim=16384/2, s=1.25992104989)
       double precision x(Ndim),psi2(Ndim),psi(Ndim),psia2(ndim),vpot(ndim)
       complex*16 im,ExpT(Ndim),ExpV(Ndim),psiA(Ndim),ExpThalf(Ndim),ExpVhalf(Ndim),ExpTfull(Ndim),ExpVfull(Ndim)
!
...
!
          ...
          ExpThalf(i) = cdexp( -im * E * dt / (2.d0-s))
          ExpTfull(i) = cdexp( im * E * dt * s / (2.d0-s))
!
...
!
          ...
          ExpThalf(N+1-i) = cdexp( -im * E * dt / (2.d0-s))
          ExpTfull(N+1-i) = cdexp( im * E * dt * s / (2.d0-s))
       END DO
!
...
!
	  ...
	  varg=alpha*x(i)
	   ...
	   vpot(i)=v
        ...
        ExpVhalf(i) = cdexp( -im * V * dt / (2*(2.d0-s)))
        ExpVfull(i) = cdexp( im * V * dt * s / (2*(2.d0-s)))
       END DO
!
...
!      call schem2A(m,N,ExpT,ExpV,psiA)
      call schem4A(m,N,ExpT,ExpV,psiA,ExpThalf,ExpVhalf,ExpTfull,ExpVfull)
...
!	   write(*,99) j*dt,dt,sumt/(sumt+sumr),(sumt+sumr)*dx
	   write(*,99) e0, sumt+sumr,sumt,sumt/(sumt+sumr),tc
	   write(14,99) e0, sumt+sumr,sumt,sumt/(sumt+sumr),tc
   end if
 200 continue
end
