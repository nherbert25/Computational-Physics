...
@author: Nate
 ...
 Use the Langevin Algorithm to do the variational Helium atom case with (in atomic units) Ψ2 T(r1,r2) = e−2αr1e−2αr2.
at α = 1.6875 and
H = −1/2∇_1^2 −1/2∇_2^2 − 2/r1 − 2/r2 + 1/r12
.
Choose a serie of time step sizes such as ∆t = 0.09,0.07,0.05,0.04,0.03,0.02,0.01 etc., until you see the linear convergence. 
...
alpha = 1.6875
time_step = [0.09,0.08,0.07,0.06,0.05,0.04,0.03,0.02,0.01,.005,.004,.003,.002,.001]
r0 = np.array([2,2,2])
r1 = np.array([3,2,2])
energy = []
std_dev = []
...
for i in range(len(time_step)):
    ...
    for j in range(N):
        ...
        r2_to_add = current_r2 - alpha*current_r2/np.linalg.norm(current_r2)*time_step[i] + np.sqrt(time_step[i])*r2_normal
