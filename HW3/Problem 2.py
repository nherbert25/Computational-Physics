...
for i in range(0,int((9*np.pi)/dt)):
	...
	r_n = use.int_q_array(r_n, v_n, .5*dt)
	t = t +.5*dt


	r1 = np.asarray([-.5*np.cos(t), -.5*np.sin(t)])
	r2 = np.asarray([.5*np.cos(t),.5*np.sin(t)])
	v_n = use.int_v_array(r_n, r1, r2, v_n, dt)
...
ax3.set_title("2D Time Dependent Two-Center Gravitational: Second Order PV", va='bottom')
plt.show()
